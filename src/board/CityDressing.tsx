...
// footprint; MOBILE loads a POSITIONAL edge-third crop — the STRAND/FLEET
// board-edge third removed, leaving a rectangular ~300×180 remainder (see
// gen-city-mobile.mjs step 1b) — and uses a single UNIFORM XZ scale keyed to the
// LONG axis so it fills the center undistorted (a small centered empty strip on
// the short axis is fine; distortion is not). Y scale is tied to the horizontal
// fit to avoid vertical distortion. City footprint is recentered at origin before
// scaling. PANs stay 0 so the bbox stays centered on the board center.
...
// city.glb byte-identical. Same buildings/UVs, minus the STRAND/FLEET board-edge
// third (mobile-only positional crop — see gen-city-mobile.mjs step 1b).
...
export function CityDressing({ isMobile = false }: { isMobile?: boolean }): React.JSX.Element {
  ...
  const { object, groupScale } = useMemo(() => {
    ...
    // MOBILE: the mobile glb has the STRAND/FLEET board-edge third removed by a
    // positional cut, leaving a rectangular ~300×180 remainder (aspect ~1.67 —
